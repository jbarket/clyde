...
**Core:** Every piece of knowledge must have single, authoritative representation.

## Key Areas
- **Code**: Extract common functions/classes
- **Config**: Centralize settings, constants
- **Business rules**: One source of truth
- **Validation**: Single validation logic per rule

## Apply When
- Logic is truly identical
- High confidence it won't diverge
- Abstraction improves clarity

## Don't Apply When  
- Coincidental duplication (different domains)
- Code likely to evolve differently
...
## Levels: Functions → Classes → Modules → Libraries → Services
...
**Core:** Break complex problems into components, analyze relationships, apply consistent methods.
...
## Tools
- **Decision Matrix**: Criteria + weights + scores = objective comparison
- **5 Whys**: Root cause analysis through systematic questioning  
